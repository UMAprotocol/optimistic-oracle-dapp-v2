 ...
 * @returns {string}
 */
export const toWei = (num: BigNumberish, decimals?: number): string =>
  toBNWei(num, decimals).toString();
...
export function getFlags(state: State): Record<Flag, boolean> {
  ...
  flags[Flag.WrongChain] =
    userChainId && requestChainId ? userChainId !== requestChainId : false;
  ...
  if (request && request.bond && request.finalFee) {
    ...
    flags[Flag.InsufficientBalance] = userCollateralBalance
      ...
      : false;
    flags[Flag.InsufficientApproval] = userCollateralAllowance
      ? userCollateralAllowance.lt(totalBond)
      : false;
  }
  ...
  if (userAddress && commands) {
    commands.forEach((command) => {
      if (
        !flags[Flag.ProposalTxInProgress] &&
        command.type === ContextType.proposePrice
      ) {
        flags[Flag.ProposalTxInProgress] = true;
      }
      if (
        !flags[Flag.DisputeTxInProgress] &&
        command.type === ContextType.disputePrice
      ) {
        flags[Flag.DisputeTxInProgress] = true;
      }
      if (
        !flags[Flag.ApprovalTxInProgress] &&
        command.type === ContextType.approve
      ) {
        flags[Flag.ApprovalTxInProgress] = true;
      }
      if (
        !flags[Flag.ChainChangeInProgress] &&
        command.type === ContextType.switchOrAddChain
      ) {
...
export const DefaultChainConfig =
  ({ getMulticall2Address }: AddressGetters) =>
  (chainId: number, chainConfig: PartialChainConfig): ChainConfig => {
...
export const DefaultConfig =
  ...
  (config: PartialConfig, oracleType: OracleType): Config => {
